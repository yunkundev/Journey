...
public class BoardManager : MonoBehaviour {
	...
	void init(){
		...
		for (int i = 0; i < number; ++i) {
			float x = (float)Random.Range (-8.25f, 8.25f);
			float y = (float)Random.Range (-4.4f, 4.0f);
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			if (hasTiles) {
				...
				if (idx == -1 || type[idx] != type[chosen[0]]) {
					...
					if (state [idx] == 1) {// has been chosen
						...
						dir = dir.normalized;
						l.transform.right = dir;
						l.GetComponentInChildren<BoxCollider2D> ().enabled = true;
